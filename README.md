...
**Citations** are sourced from `.bib` files: any file in your working directory is picked up automatically, and you can point citeref at a global library too. A fuzzy picker (fzf-lua, telescope, snacks) lets you search by key, title, author, or journal with a live preview; a completion backend (blink.cmp, nvim-cmp) offers the same entries inline as you type `@` or `\*cite*{`.

**Cross-references** are generated by scanning the named `{r}` chunks in the current buffer and its sibling `.rmd`/`.qmd` files. Select a chunk and citeref inserts the right reference syntax for your document type — `\@ref(fig:label)` for R Markdown, or `@label` for Quarto's native cross-reference format.
...
citeref recognises both chunk labelling styles:

**R Markdown / knitr inline label** (label on the fence line):
...
**Quarto YAML options** (label as a `#|` comment inside the chunk):
...
# referenced as @myplot
```
````

Both styles are detected and shown together in the picker, regardless of which file type you are editing.
...
````markdown
```{r}
# unnamed — add a label (inline or via #| label:) to use in a cross-reference
```
````
