...
describe("citation_under_cursor", function()
  ...
  local function set_cursor_on_needle(line_text, needle)
    local s = line_text:find(needle, 1, true)
    assert.is_not_nil(s)
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, { line_text })
    vim.api.nvim_win_set_cursor(0, { 1, s - 1 })
  end
  ...
  it("detects a MyST citation when cursor is on the key", function()
    set_cursor_on("See {cite:p}`smith2020`.", 14)
    local info = parse.citation_under_cursor()
    assert.is_not_nil(info)
    assert.equals("smith2020", info.key)
    assert.equals("myst", info.style)
    assert.equals("cite:p", info.cmd)
  end)

  it("detects MyST prefix and suffix wrappers", function()
    set_cursor_on("See {cite:t}`{see}smith2020{fig 1}`.", 20)
    local info = parse.citation_under_cursor()
    assert.is_not_nil(info)
    assert.equals("see", info.prefix)
    assert.equals("fig 1", info.suffix)
    assert.equals("cite:t", info.cmd)
  end)

  it("detects the exact MyST key under the cursor in multi-key citations", function()
    set_cursor_on_needle("See {cite:p}`abreu2020; ackerly2006`.", "ackerly2006")
    local info = parse.citation_under_cursor()
    assert.is_not_nil(info)
    assert.equals("ackerly2006", info.key)
    assert.same({ "abreu2020", "ackerly2006" }, info.all_keys)
  end)
