...
function M.get()
  ...
  if not _registry[name] then
    ...
    if ok then
      ...
      vim.notify(
        "citeref: backend '" .. name .. "' could not be loaded.\n" .. tostring(mod),
...
function M.call(fn_name, ...)
  ...
  if not b then
    vim.notify(
      "citeref: no backend configured. Add backend = '...' to your setup() call.",
      vim.log.levels.WARN
    )
    return
  end
  if type(b[fn_name]) ~= "function" then
    vim.notify(
      "citeref: backend '" .. tostring(require("citeref.config").get().backend)
      .. "' does not support '" .. fn_name .. "'.",
      vim.log.levels.WARN
    )
    return
  end
