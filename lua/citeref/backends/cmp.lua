...
local function reset() _mode = "all" ; _format = "markdown" end
...
local function current_items()
  if _mode == "citation"     then return citation_items(_format) end
  if _mode == "crossref_fig" then return crossref_items("fig")   end
  if _mode == "crossref_tab" then return crossref_items("tab")   end
  local items = {}
...
function Source:get_debug_name() return "citeref" end
...
function M.show(mode, format)
  ...
  vim.api.nvim_create_autocmd("User", {
    pattern = "CmpMenuClosed", once = true, callback = reset,
  })
end
